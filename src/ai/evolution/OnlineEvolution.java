...
public class OnlineEvolution implements AI, AiVisualizor {
	...
	public List<Double> championFindGen;
	public int curr_championFindGen = 0;
	...
	public OnlineEvolution(boolean useHistory, int popSize, double mutRate, double killRate, int budget, IStateEvaluator evaluator, boolean stepped, boolean saveStats) {
		...
		championFindGen = new ArrayList<Double>();
	}
...
//		System.out.println(Thread.currentThread().getName() + " Online Evolution real cpu time: " + (end - start));
		...
		if(saveStats) {
			...
			championFindGen.add((double)curr_championFindGen);
